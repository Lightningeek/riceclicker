...
(function(){
	...
	*/
	...
	//upgrades and achievements can use other pictures than icons.png; declare their icon with [posX,posY,'c]
...
Game.Launch=function()
{
	...
	'<div class="title">2/27/2023 - It\'s broken, I\'m sad</div>'+
	...
	Game.Init=function()
	{
		...
		Game.WriteSave=function(type)
		{
			...
			(type==3?'\n	soy sauce droplet type : ':'')+parseInt(Math.floor(Game.lumpCurrentType))+';'+
		...
		Game.lumpTooltip=function()
		{
			...
			loc("You have %1 soy sauce droplets.",'<span class="price lump">'+loc (LBeautify(Game.lumps) + " soy sauce droplets",LBeautify(Game.lumps))+'</span>')+
			'<div class="line"></div>'+
		...
		//define objects
		new Game.Object('Chopsticks','cursor|chopsticks|clicked|[X] extra finger|[X] extra fingers','Automatically once every 10 seconds.',0,0,{},15,function(me){
			var add=0;
			if (Game.Has('Thousand fingers')) add+=		0.1;
			if (Game.Has('Million fingers')) add*=		5;
			if (Game.Has('Billion fingers')) add*=		10;
		...
		new Game.Object('Grandma','grandmas|老爷|baked|grandmas are [X] year older|老爷 are [X] years older','A nice 老爷 to cook more rice.',1,1,{pic:function(i){
			var list=['grandma'];
		...
		new Game.Object('Farm','farm|rice farms|harvested|[X] more acre|[X] more acres','Be honest, you know what this does.',3,2,{base:'farm',xV:8,yV:8,w:64,rows:2,x:0,y:16},500,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
		...
		Game.last.minigameName=loc("Grandpa's bonsai garden");
		
		new Game.Object('Mine','mine|mines|mined|[X] mile deeper|[X] miles deeper','Mines rice from the depths of asia.',4,3,{base:'mine',xV:16,yV:16,w:64,rows:2,x:0,y:24},10000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		...
		new Game.Object('Factory','factory|factories|mass-produced|[X] additional patent|[X] additional patents','Produces industrial quantities of rice cookers.',5,4,{base:'factory',xV:8,yV:0,w:64,rows:1,x:0,y:-22},3000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		...
		Game.last.minigameName=loc("Stonk Market");
		...
		new Game.Object('Prism','prism|prisms|converted|[X] new color discovered|[X] new colors discovered','Converts light itself into rice.',14,14,{base:'prism',xV:16,yV:4,w:64,rows:1,x:0,y:20},75000000000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		...
		new Game.Object('Chancemaker','chancemaker|chancemakers|spontaneously generated|Chancemakers are powered by [X]-leaf clovers|Chancemakers are powered by [X]-leaf clovers','Generates Rice out of thin air through sheer luck.',15,19,{base:'chancemaker',xV:8,yV:64,w:64,rows:1,x:0,y:0,rows:2},77777777777,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		...
		Game.foolObjects={
			...
			'Chopsticks':{name:'Rolling pin',desc:'Essential in stabbing the people who try to rob you of your rice. The first step in rice-making.',icon:0},
			'Grandma':{name:'Oven',desc:'A crucial element of cooking rice.',icon:1},
		...
		order=200;
		Game.TieredUpgrade('Forwards from 爷爷','<q>加油加油!</q>','Grandma',1);
		...
		order=300;
		Game.TieredUpgrade('Cheap hoes','<q>Rake in the rice!</q>','Farm',1);
		Game.TieredUpgrade('Fertilizer','<q>It\'s brown rice, I swear.</q>','Farm',2);
		Game.TieredUpgrade('Cookie trees','<q>A relative of the Rice fruit.</q>','Farm',3);
		
		order=500;
		Game.TieredUpgrade('Sturdier conveyor belts','<q>You\'re going places.</q>','Factory',1);
		Game.TieredUpgrade('Child labor','<q>Where all the disappointing children who get B\'s go to</q>','Factory',2);
		Game.TieredUpgrade('Sweatshop','<q>Wait! I thought this was a humane one!.</q>','Factory',3);
		
		order=400;
		Game.TieredUpgrade('Sugar gas','<q>A white, volatile gas, found in the depths of some arborio rice caves.</q>','Mine',1);
		Game.TieredUpgrade('Megadrill','<q>You\'re in deep, in the met- I meant math buisness.</q>','Mine',2);
		...
		Game.TieredUpgrade('Essence of dough','<q>Best way to go Breaking Bad Asain style, If you know what I mean</q>','Alchemy lab',2);
		...
		order=15000;
		new Game.Upgrade('Specialized chocolate chips',getStrCookieProductionMultiplierPlus(1)+'<q>Computer-designed rice grains, comes in gold, silver, and bronze. Computer chips, if you will.</q>',1000000000000000,[0,9],function(){Game.SetResearch('Designer cocoa beans');});Game.last.pool='tech';
		...
		new Game.Upgrade('Turkey egg',getStrCookieProductionMultiplierPlus(1)+'<br>'+loc("Cost scales with how many eggs you own.")+'<q>Olive garden isn\'t really an American resturant, but it might as well be.</q>',eggPrice,[3,12]);
